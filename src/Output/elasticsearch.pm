...
sub init {
    ...
    if ($response->{_rc} == 200) {
        ...
        my $response = $ua->put($cfg->{url}.'/_template/pcap', 'Content-type' => 'application/json', 'Content' => encode_json \%EStemplate);
...
sub push {
    ...
    print 'Sending to '.$cfg->{url}.'/'.$index.' : '.$data."\n" if ($cfg->{debug} eq 'true');
    my $response = $ua->post($cfg->{url}.'/'.$index.'/_doc', 'Content-type' => 'application/json', 'Content' => $data);
    if ($response->is_success) {
